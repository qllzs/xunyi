...
void ANO_OLED_R_SPI_Init(void)
{
	R_SPI_InitTypeDef R_SPI_InitStructure; 
	GPIO_InitTypeDef GPIO_InitStructure;
	
	RCC_APB2PeriphClockCmd(OLED_RCC_SPI_GPIO_SPI, ENABLE);
	...
	GPIO_Init(ANO_OLED_SPI_GPIO_SPI, &GPIO_InitStructure);
	
	R_SPI_InitStructure.SPI_Direction = SPI_Direction_1Line_Tx; 
	R_SPI_InitStructure.SPI_Mode = SPI_Mode_Master; //主模式 
	R_SPI_InitStructure.SPI_DataSize = SPI_DataSize_8b; //数据大小8位 
	R_SPI_InitStructure.SPI_CPOL = SPI_CPOL_High; 
	R_SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge; 
	R_SPI_InitStructure.SPI_NSS = SPI_NSS_Soft; //NSS信号由软件产生 
	R_SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2; 
	R_SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB; //高位在前 
	R_SPI_InitStructure.SPI_CRCPolynomial = 0; 
